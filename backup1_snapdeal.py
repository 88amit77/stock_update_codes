...
def main():
    ...
    # master files
    # master_filename = 'master_refresh_update11_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
    ...
    #
    # # snapdeal files
    # snapdeal_filename = 'master_refresh_snapdeal_update11_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
    ...
    #products_query = "SELECT product_id, buymore_sku from master_masterproduct where product_id in ('234663','238767','249547','234671','243290','249496','251295')"
    ##for hard code testing
    #products_query = "SELECT product_id, buymore_sku from master_masterproduct where product_id = 234663 and buymore_sku = 'FOOTLOOSE_F1101W8ENBL'"
    ...
    u_index = interval
    current_l = []
    while l_index <= len(l) - 1:
            master_filename = 'master_refresh_update__20_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
            master_filefrom = '/tmp/' + master_filename
            master_fileto = '/buymore2/bin_reco/master_stock/' + master_filename
            master_update_csv = open(master_filefrom, 'w')
            master_fieldnames = ['buymore_sku', 'product_id', 'warehouse', 'stock', 'status', 'error']
            ...
            # snapdeal files
            snapdeal_filename = 'master_refresh_snapdeal_update20_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
            snapdeal_filefrom = '/tmp/' + snapdeal_filename
            snapdeal_fileto = '/buymore2/bin_reco/master_stock/' + snapdeal_filename
            snapdeal_update_csv = open(snapdeal_filefrom, 'w')
            snapdeal_fieldnames = ['product_id', 'quantity', 'message', 'status']
            ...
            current_l = l[l_index:u_index]
            l_index = u_index
            u_index = u_index + interval
            ...
            # for item in binreco:
                ...
                print("348snapdeal_set",snapdeal_set)
                ...
                print("snapdeal_set===>>>",snapdeal_set)
                ...
                if page < total_pages:
                    ...
                    if 'message' in res['metadata']:
                        ...
                        for result in results:
                            print("result===",result)
                            snapdeal_csv_row = {
                                'product_id': str(result['supc']),
                                'quantity': str(snapdeal_quantity[upload_id][result['supc']]),
                                'message': upload_id,
                                'status': result['state']
                            }
                            snapdeal_writer.writerow(snapdeal_csv_row)
