...
def get_snapdeal_accounts(cur_products):
    ...
    if portal is not None:
        ...
        # cur_products.execute("Select * from master_portalaccountdetails where portal_id_id = " + str(portal_id)+" and account_id = 12")
        cur_products.execute(
            "Select * from master_portalaccountdetails where portal_id_id = " + str(portal_id))
...
def main():
    ...
    while l_index <= len(l) - 1:
        ...
        # Create files for the logs of the stock update
        # master files
        master_filename = 'master_refresh_update12_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
        master_filefrom = '/tmp/' + master_filename
        master_fileto = '/buymore2/bin_reco/master_stock/' + master_filename
        master_update_csv = open(master_filefrom, 'w')
        master_fieldnames = ['buymore_sku', 'product_id', 'warehouse', 'stock', 'status', 'error']
        ...
        # snapdeal files
        snapdeal_filename = 'master_refresh_snapdeal_update12_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
        snapdeal_filefrom = '/tmp/' + snapdeal_filename
        snapdeal_fileto = '/buymore2/bin_reco/master_stock/' + snapdeal_filename
        snapdeal_update_csv = open(snapdeal_filefrom, 'w')
        snapdeal_fieldnames = ['product_id', 'quantity', 'message', 'status']
        ...
        for item in current_l:
            ...
            #     warehouse_id)
            query = "SELECT * from api_masterstock where product_id = " + str(product_id) + " and warehouse= 10"
