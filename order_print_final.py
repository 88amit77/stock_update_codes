...
def lambda_handler(event, context):
    ...
    for account in accounts:
        for warehouse in account['warehouses']:
            cur_orders.execute(
                'Select no.dd_id, no.order_id, no.order_item_id, dd.fulfillment_model from api_neworder no inner join api_dispatchdetails dd on no.dd_id = dd.dd_id_id where no.portal_id = 8 and dd.is_mark_placed = False and warehouse_id = ' + str(
